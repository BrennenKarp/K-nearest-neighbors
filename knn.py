...
def main():
  ...
  #Test out our prediction code
    ...
    for k in [1,3,5,7,9,99,999,8000]:
      ...
      #######################################
      predicted = predict(train_X, train_y, train_X, k)
      train_acc = compute_accuracy(train_y, predicted) 
...
def get_nearest_neighbors(example_set, query, k):
    sorted_args = np.array([])
    dist_list = np.linalg.norm(example_set - query, axis = 1)
    sorted_args = np.argsort(dist_list)
...
def knn_classify_point(examples_X, examples_y, query, k):
    ...
    for x in indexes:
        predicted_vals = np.append(predicted_vals, examples_y[x])
    predicted_label = np.round(np.mean(predicted_vals))
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
    split = np.split(train_X, num_folds)
    split_labels = np.split(train_y, num_folds)
    val_acc = np.array([])
    for i in range(num_folds):
        train = np.vstack(split[:i] + split[i+1:])
        train_labels = np.vstack(split_labels[:i] + split_labels[i+1:])
        evaluate = split[i]
        evaluate_labels = split_labels[i]
        predicted = predict(train, train_labels, evaluate, k)
        val_acc = np.append(val_acc, compute_accuracy(evaluate_labels, predicted))
    avg_val_acc = np.mean(val_acc)
    varr_val_acc = np.var(val_acc)
...
#Question 9 Answers
#using the entire training set k = 1 seems to be the best number of neighbors, however when we use
#cross validation which is a more effective way of doing it, it shows that k = 99 is the best number of 
#neighbors. When k = 1 the training error is not 0% but pretty close. This is due to the fact that it's
#extrememly difficult to get a model that has 0% training error due to the variablity of real life data.
#For training accuracy I noticed that it starts high at k = 1 and then goes down sharply at first when you 
#raise k, then as you continue to raise k it still drops but a lot slower. For val accuracy the distribution with
#respect to k seems to be gaussian. The highest percentage is in the middle of the k values at k = 99. This is a good
#example of underfitting when k = 1. The training acc is high, however when we validate it the accuracy is the lowest, due to 
#the fact that we underfit the data. 

#Debrief
#I spent about 10 hours in total on this assignment
#I would rate this assignment as moderate
#I worked on it alone but with help from discord
#I feel like i understand the material very well now. Besides a few of the numpy specifics. I'd say 90%
#Nope.

#Kaggle name: Brennen J Karp